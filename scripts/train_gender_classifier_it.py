...
corpus_path = r"gc_training_data\it"

# Load dataset from files
train = []
with open(os.path.join(corpus_path, r'train.csv'), 'r', encoding='utf8') as f:
    lines = f.readlines()
    for line in lines[1:]:
      sent = line.split("\t")[0]
      label = line.split("\t")[1]
...
val = []
with open(os.path.join(corpus_path, r'val.csv'), 'r', encoding='utf8') as f:
    lines = f.readlines()
    for line in lines[1:]:
      sent = line.split("\t")[0]
      label = line.split("\t")[1]
...
test = []
with open(os.path.join(corpus_path, r'test.csv'), 'r', encoding='utf8') as f:
    lines = f.readlines()
    for line in lines[1:]:
      sent = line.split("\t")[0]
      label = line.split("\t")[1]
...
def tokenize(dataset):
  ...
  for sample in dataset:
    tokenized_sent = word_tokenize(sample[0], language='italian')
...
w2v_vectors = KeyedVectors.load(r"embeddings\word2vec_it\W2V.kv")
...
w2v_train = embed(train, w2v_vectors, 'italian')
w2v_val = embed(val, w2v_vectors, 'italian')
w2v_test = embed(test, w2v_vectors, 'italian')
...
cnn_w2v_tuner.search(w2v_xtrain, y_train,
                     callbacks=[
                        ...
                        keras.callbacks.TensorBoard(log_dir=r"tensorboard/cnn_w2v_it")
                     ],
...
# Export model
cnn_w2v.save(r"models\cnn_w2v_it.keras")
cnn_w2v.save_weights(r"models\cnn_w2v_it.weights.h5")
...
lstm_w2v_tuner.search(w2v_xtrain, y_train,
                      callbacks=[
                         ...
                         keras.callbacks.TensorBoard(log_dir=r"tensorboard/lstm_w2v_it")
                      ],
                      validation_data=(w2v_xval,  y_val))
...
# Export model
lstm_w2v.save(r"models\lstm_w2v_it.keras")
lstm_w2v.save_weights(r"models\lstm_w2v_it.weights.h5")


### 2. fastText
# Import pre-trained vectors
from gensim.models.fasttext import load_facebook_vectors
ft_vectors = load_facebook_vectors(r"embeddings\cc.it.300.bin.gz",
                                   encoding='utf-8')

ft_train = embed(train, ft_vectors, 'italian')
ft_val = embed(val, ft_vectors, 'italian')
ft_test = embed(test, ft_vectors, 'italian')
...
cnn_ft_tuner.search(ft_xtrain, y_train,
                    callbacks=[
                       keras.callbacks.EarlyStopping("val_loss", patience=1, restore_best_weights=True),
                       keras.callbacks.TensorBoard(log_dir=r"tensorboard\cnn_ft_it")
                    ],
...
# Export model
cnn_ft.save(r"models\cnn_ft_it.keras")
cnn_ft.save_weights(r"models\cnn_ft_it.weights.h5")
...
lstm_ft_tuner.search(ft_xtrain, y_train,
                     callbacks=[
                        keras.callbacks.EarlyStopping("val_loss", patience=2, restore_best_weights=True),
                        keras.callbacks.TensorBoard(log_dir=r"tensorboard/lstm_ft_it")
                     ],
                     validation_data=(ft_xval,  y_val))
...
# Export model
lstm_ft.save(r"models\lstm_ft_it.keras")
lstm_ft.save_weights(r"models\lstm_ft_it.weights.h5")
...
## 4.1. CNN
# Instantiate tuner
cnn_roberta_tuner = keras_tuner.Hyperband(
    ...
    overwrite=True,
    directory=r"keras_tuners",
    project_name=r"cnn_roberta_it"
)

# Search best hyperparameters
cnn_roberta_tuner.search(roberta_x_train, y_train,
                     callbacks=[
                        keras.callbacks.EarlyStopping("val_loss", patience=1, restore_best_weights=True),
                        keras.callbacks.TensorBoard(log_dir=r"tensoboard\cnn_roberta_it")
                     ],
                     validation_data=(roberta_x_val, y_val))

# Retrieve best hyperparameters
best_hps = cnn_roberta_tuner.get_best_hyperparameters()[0]
print(best_hps.values)

# Retrieve best model
cnn_roberta = cnn_roberta_tuner.get_best_models()[0]
cnn_roberta.summary()
...
cnn_roberta.save(r"models\cnn_roberta_it.keras")
cnn_roberta.save_weights(r"models\cnn_roberta_it.weights.h5")
...
lstm_roberta_tuner = keras_tuner.Hyperband(
    ...
    overwrite=True,
    directory=r"keras_tuners",
    project_name=r"lstm_roberta_it"
)
...
lstm_roberta_tuner.search(roberta_x_train, y_train,
                     callbacks=[
                        keras.callbacks.EarlyStopping("val_loss", patience=1, restore_best_weights=True),
                        keras.callbacks.TensorBoard(log_dir=r"tensorboard\cnn_roberta_it")
                     ],
                     validation_data=(roberta_x_val, y_val))
...
## Export model
lstm_roberta.save(r"models\lstm_roberta_it.keras")
lstm_roberta.save_weights(r"models\lstm_roberta_it.weights.h5")
...
# using RoBERTa
# with maximum sequence length set at 100
umberto_tokenizer = AutoTokenizer.from_pretrained('Musixmatch/umberto-commoncrawl-cased-v1')

umberto_train_tokenized = umberto_tokenizer(x_train, padding=True, truncation=True,
                                     max_length = 100, return_tensors='tf')
umberto_val_tokenized = umberto_tokenizer(x_val, padding=True, truncation=True,
                                     max_length = 100, return_tensors='tf')
umberto_test_tokenized = umberto_tokenizer(x_test, padding=True, truncation=True,
                                     max_length = 100, return_tensors='tf')
...
umberto_model = TFAutoModel.from_pretrained("Musixmatch/umberto-commoncrawl-cased-v1")

batch_size = 64

train_embeddings = generate_embeddings(umberto_model, umberto_train_tokenized, batch_size)
val_embeddings = generate_embeddings(umberto_model, umberto_val_tokenized, batch_size)
test_embeddings = generate_embeddings(umberto_model, umberto_test_tokenized, batch_size)

umberto_x_train = tf.concat(train_embeddings, axis=0)
print('UmBERTo train tensor shape: ', roberta_x_train.shape)
umberto_x_val = tf.concat(val_embeddings, axis=0)
print('UmBERTo validation tensor shape: ', roberta_x_val.shape)
umberto_x_test = tf.concat(test_embeddings, axis=0)
print('Test tensor shape: ', roberta_x_test.shape)
...
print('Max sequence length: ', maxlen)
roberta_embedding_dims = roberta_x_train.shape[2]
print('UmBERTo word embedding dimensions: ', roberta_embedding_dims)

## 4.1. CNN
# Instantiate tuner
cnn_umberto_tuner = keras_tuner.Hyperband(
    hypermodel = CNN_HyperModel(maxlen=maxlen, embedding_dims=roberta_embedding_dims),
    objective="val_loss",
    max_epochs=15,
    seed=42,
    overwrite=True,
    directory=r"keras_tuners",
    project_name=r"cnn_umberto_it"
)

# Search best hyperparameters
cnn_umberto_tuner.search(umberto_x_train, y_train,
                     callbacks=[
                        keras.callbacks.EarlyStopping("val_loss", patience=1, restore_best_weights=True),
                        keras.callbacks.TensorBoard(log_dir=r"tensoboard\cnn_umberto_it")
                     ],
                     validation_data=(roberta_x_val, y_val))

# Retrieve best hyperparameters
best_hps = cnn_umberto_tuner.get_best_hyperparameters()[0]
print(best_hps.values)

# Retrieve best model
cnn_umberto = cnn_umberto_tuner.get_best_models()[0]
cnn_umberto.summary()

## Evaluate best model
# Get predictions
predictions = cnn_umberto.predict(umberto_x_test)
# Binarize
y_pred = []
for pred in predictions:
  y_pred.append((pred > 0.5).astype(int))
# Accuracy
print('Accuracy score for CNN with UmBERTo embeddings:', metrics.accuracy_score(y_test, y_pred), '\n')
# Main classification metrics
print('Classification report for CNN with UmBERTo embeddings')
print(metrics.classification_report(y_test, y_pred, zero_division=0.0), '\n')
# Confusion matrix
print('Confusion matrix for CNN with UmBERTo embeddings')
print(pd.DataFrame(metrics.confusion_matrix(y_test, y_pred)))

## Export re-trained model
cnn_umberto.save(r"models\cnn_umberto_it.keras")
cnn_umberto.save_weights(r"models\cnn_umberto_it.weights.h5")

## 4.2. LSTM
## Tune model hyperparameters
# Instantiate tuner
lstm_umberto_tuner = keras_tuner.Hyperband(
    hypermodel = LSTM_HyperModel(maxlen=maxlen, embedding_dims=roberta_embedding_dims),
    objective="val_loss",
    max_epochs=15,
    seed=42,
...
# Search best hyperparameters
lstm_umberto_tuner.search(roberta_x_train, y_train,
                     callbacks=[
                        keras.callbacks.EarlyStopping("val_loss", patience=1, restore_best_weights=True),
                        keras.callbacks.TensorBoard(log_dir=r"tensorboard\lstm_umberto_it")
                     ],
                     validation_data=(roberta_x_val, y_val))

# Retrieve best hyperparameters
best_hps = lstm_umberto_tuner.get_best_hyperparameters()[0]
print(best_hps.values)

# Retrieve best model
lstm_umberto = lstm_umberto_tuner.get_best_models()[0]
lstm_umberto.summary()

## Export model
lstm_umberto.save(r"models\lstm_umberto_it.keras")
lstm_umberto.save_weights(r"models\lstm_umberto_it.weights.h5")